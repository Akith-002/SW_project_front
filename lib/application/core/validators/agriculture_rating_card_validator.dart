class AgricultureRatingCardValidator {
  ...
  /// Validates plantation age field (must be between 0 and 200 years)
  static String? validatePlantationAge(String? value, String fieldName) {
    if (value == null || value.trim().isEmpty) {
      return '$fieldName is required';
    }
    final number = int.tryParse(value);
    ...
    if (number > 200) {
      return '$fieldName cannot be more than 200 years';
    }
    return null;
  }
  ...
  /// Validates land area fields (positive decimals with 2 decimal places)
  static String? validateLandArea(String? value, String fieldName) {
    if (value == null || value.trim().isEmpty) {
      return '$fieldName is required';
    }
    final number = double.tryParse(value);
  ...
  /// Validates yield per acre (positive number)
  static String? validateYield(String? value, String fieldName) {
    if (value == null || value.trim().isEmpty) {
      return '$fieldName is required';
    }
    final number = double.tryParse(value);
