class ShopsRatingCardValidator {
  ...
  /// Validates floor area (positive number)
  static String? validateFloorArea(String? value, String fieldName) {
    if (value == null || value.trim().isEmpty) {
      return '$fieldName is required';
    }
    final number = double.tryParse(value);
  ...
  /// Validates shop number/unit
  static String? validateShopNumber(String? value, String fieldName) {
    if (value == null || value.trim().isEmpty) {
      return '$fieldName is required';
    }
    final regex = RegExp(r'^[a-zA-Z0-9\-/]+$');
    if (!regex.hasMatch(value)) {
      return '$fieldName must contain only letters, numbers, hyphens, and slashes';
    }
    return null;
  }
}