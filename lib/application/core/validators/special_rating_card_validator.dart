class SpecialRatingCardValidator {
  ...
  /// Validates age field (must be between 0 and 300 for special properties)
  static String? validateAge(String? value, String fieldName) {
    ...
    if (number > 300) {
      return '$fieldName cannot be more than 300 years';
    }
    return null;
  }
  ...
  /// Validates capacity field (positive integer)
  static String? validateCapacity(String? value, String fieldName) {
    if (value == null || value.trim().isEmpty) {
      return '$fieldName is required';
    }
    final number = int.tryParse(value);
  ...
  /// Validates special property identifier
  static String? validatePropertyIdentifier(String? value, String fieldName) {
    if (value == null || value.trim().isEmpty) {
      return '$fieldName is required';
    }
    final regex = RegExp(r'^[a-zA-Z0-9\-/_]+$');
    if (!regex.hasMatch(value)) {
      return '$fieldName must contain only letters, numbers, hyphens, underscores, and slashes';
    }
    return null;
  }

  /// Validates area measurements (can be very large for special properties)
  static String? validateArea(String? value, String fieldName) {
    if (value == null || value.trim().isEmpty) {
      return '$fieldName is required';
    }
    final number = double.tryParse(value);
